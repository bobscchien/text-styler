...
if __name__ == '__main__':
    ...
    train_texts, train_labels = pd.read_csv(os.path.join(configuration.DIR_INTERMIN, train_file, 'train.zip'))[['source', 'target']].dropna().values.T.tolist()
    valid_texts, valid_labels = pd.read_csv(os.path.join(configuration.DIR_INTERMIN, train_file, 'valid.zip'))[['source', 'target']].dropna().values.T.tolist()
    test_texts, test_labels = pd.read_csv(os.path.join(configuration.DIR_INTERMIN, train_file, 'test.zip'))[['source', 'target']].dropna().values.T.tolist()    
    ...
    file_path = os.path.join(configuration.DIR_TFRECORD, train_file)
